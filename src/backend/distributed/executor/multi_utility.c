...
static void AcquireDistributedLockOnRelations(List *relationIdList,
											  LOCKMODE localLockMode,
											  char *remoteLockMode);
 ...
 * Both local and remote lock modes are expected to be of same type. Reason
 * we have both here is that we needed the ENUM for local locking function, and
 * string representation for remote locking function.
 *
 ...
 * and previous workers that has metadata sync turned on. It does not know
 * about nodes that have metadata sync turned on afterwards.
 */
static void
AcquireDistributedLockOnRelations(List *relationIdList, LOCKMODE localLockMode,
								  char *remoteLockMode)
{
	ListCell *relationIdCell = NULL;
	...
	foreach(relationIdCell, relationIdList)
	{
		...
		if (ShouldSyncTableMetadata(relationId))
		{
			...
			appendStringInfo(lockRelationCommand, LOCK_RELATION_IF_EXISTS,
							 qualifiedRelationName, remoteLockMode);
			...
			foreach(workerNodeCell, workerNodeList)
			{
				...
				{
					LockRelationOid(relationId, localLockMode);
					continue;
				}
